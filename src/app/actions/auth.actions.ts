...
export async function registerUser(prevState: ResponseResult, formData: FormData): Promise<ResponseResult>{
    ...
    if(!name || !email || !password){
        logEvent(
                'Validation error: Missing fields for registration',
                'auth',
                { name, email},
                'warning',
            );
    ...
    if(existingUser){
        logEvent(
                `Registrtation failed: User already exists with this email - ${email}`,
                'auth',
                { email},
                'warning',
            );
    ...
    return { success: true, message: 'Registration has been completed successfully'}

    }catch (error){
        logEvent(
            `Unexpected error occured during registration`,
            'auth',
            {},
            'error',
            error
